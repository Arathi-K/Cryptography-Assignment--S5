...
int main ()
{
  ...
  getline(file,ct);
  file.close();
  cout<<"Cipher text received from file is "<<ct<<endl;
  ct = hex2bin (ct);
  string lct = "";
  string rct = "";
  initial_permutation_andsplitting (ct, lct, rct);
//   cout << bin2hex(pt) <<"            " << bin2hex(lpt) <<"    " << bin2hex(rpt)<< endl;

  int num;

  vector<string> roundkeysbinary;
  file.open("desroundkey.txt");
  string line;
  while(file.good()){
    getline(file,line);
    roundkeysbinary.push_back(line);
  }
  file.close();
  
  reverse(roundkeysbinary.begin(), roundkeysbinary.end()-1);
  for (int i = 1; i <= 16; i++)
    {
        cout<<"round "<<i<<": ";
        // cout<<"hii"<<roundkeysbinary[0]<<endl;
        desround (lct, rct,roundkeysbinary[i-1], i);
        cout<<"Middle text: "<<bin2hex(lct)+bin2hex (rct)<< " key : "<<bin2hex(roundkeysbinary[i-1])<<endl;
    }
  ...
  // Final Permutation
  string pt = bin2hex (permute (combine, final_perm, 64));
  cout << "\nPlain Text: " << pt << endl;
