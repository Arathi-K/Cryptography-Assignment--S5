...
//   cout<<"Permutation is "<<bin2hex(per)<<endl;
  return per;
}
...
void fun_64_to_56 (string &key, string &lk, string &rk)	// Parity bit drop table
{
...
void
initial_permutation_andsplitting (string & plaintxt, string &lpt,string & rpt)
{
  // cout<<"inside fun pt:"<<plaintxt<<endl;
  ...
  // Initial Permutation
  plaintxt = permute (plaintxt, initial_perm, 64);

  // Splitting
  lpt = plaintxt.substr (0, 32);
  rpt = plaintxt.substr (32, 32);

}

string
func (string s, string k)
{
  //s=>32
  int expansion_p[48] = { 32, 1, 2, 3, 4, 5, 4, 5, 6, 7, 8, 9,
    8, 9, 10, 11, 12, 13, 12, 13, 14, 15, 16, 17,
    16, 17, 18, 19, 20, 21, 20, 21, 22, 23, 24, 25,
    24, 25, 26, 27, 28, 29, 28, 29, 30, 31, 32, 1
  };

  string expanded_s = permute (s, expansion_p, 48);
  string x = xor_ (expanded_s, k);
  // S-box Table
  int sbox[8][4][16] = {
  ...
  for (int i = 0; i < 8; i++)
    {
      ...
      int col =
	8 * int (x[i * 6 + 1] - '0') + 4 * int (x[i * 6 + 2] - '0') +
...
//              op="12345678123456781234567812345678";  
  // Straight D-box
  return permute (op, per, 32);


}

void desround (string & lpt, string & rpt, string key, int roundnum)
{
  //mixer
  lpt = xor_ (lpt, func (rpt, key));
...
int main ()
{
  ...
  string lpt = "";
  string rpt = "";
  initial_permutation_andsplitting (pt, lpt, rpt);
//   cout << bin2hex(pt) <<"            " << bin2hex(lpt) <<"    " << bin2hex(rpt)<< endl;

  key = hex2bin (key);
    
  string lk = "", rk = "";	//28 bit left and right keys
  fun_64_to_56 (key, lk, rk);
  
//   cout<<"key is "<<bin2hex(key)<<"    lk is "<<bin2hex(lk)<<"    rk is "<<bin2hex(rk)<<endl;
  ...
  for (int i = 1; i <= 16; i++)
	...
	num = 2;
    //   cout<<"before shifting lk is "<<bin2hex(lk)<<"    rk is "<<bin2hex(rk)<<endl;
      lk = shift_left (lk, num);
      rk = shift_left (rk, num);
    //   cout<<"after shifting lk is "<<bin2hex(lk)<<"    rk is "<<bin2hex(rk)<<endl;
      string key = lk + rk;
      // Key Compression
      key = permute (key, key_comp, 48);
    //   cout<<"key is "<<bin2hex(key)<<endl;
      desround (lpt, rpt, key, i);
      roundkeysbinary.push_back(key);
     cout<<"Middle text: "<<bin2hex(lpt)+bin2hex (rpt)<< " key : "<<bin2hex(key)<<endl;
    }
  ...
  myFile<<cipher;
  myFile.close();
  myFile.open("desroundkey.txt");
  for(int i=0;i<roundkeysbinary.size();i++){
      myFile<<roundkeysbinary[i]<<endl;
  }
